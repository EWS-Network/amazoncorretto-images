...
ADD https://yum.corretto.aws/corretto.key /opt/corretto.key
ADD https://yum.corretto.aws/corretto.repo /etc/yum.repos.d/corretto.repo
RUN yum upgrade -y;\
    rpm --import /opt/corretto.key ;\
    yum install -y \
